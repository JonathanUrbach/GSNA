...
#
# # This function maps tick_values to cell values.
# #
# # Arguments:
# #   tick_values: A vector of values for ticks generated using the axisTicks function, either for log-scale or
# #                linear scale.
# #   cell_values: A vector of length *n* containing corresponding to x or y scale values of each cell in the
# #                legend color matrix. These may be log or linear scale.
# #   n: the number of values in tick_values
# #
# # Cell values in the legend color matrix may only approximate values generated by the tick_values function.
# # This function maps the tick values to the nearest cell values and returns a character vector containing
# # containing tick_values at the indices that correspond to the nearest cell values.
# map_ticks_to_cells <- function( tick_values, cell_values, n = NULL) ){
#   if( is.null( n ) ) n <- length( tick_values )
#   #tick_values <- axisTicks( usr = log10( numbers.domain ), log = TRUE )
#   #cell_vals <- 10 ** seq( log10( numbers.domain[1] ), log10( numbers.domain[2] ), length.out = n )
#   ticks.at <- sapply( X = tick_values,
#                       FUN = function( x ){
#                         .diff.abs <- abs( cell_values - x )
#                         which( .diff.abs == min(.diff.abs) )[1]
#                       })
#   #ticks <- rep(x = NA, n)
#   ticks <- rep(x = "", n)
#   for( i in 1:length( tick_values ) ){
#     ticks[ticks.at[i]] <- as.character(tick_values[i])
#   }
#   ticks
# }
...
make2ColorLegend.old <- function(numbers.1,
                             numbers.2,
                             twoColorEncode.fun,
                             n = 100,
