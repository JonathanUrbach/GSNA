...
#'
#' @returns Either a **ggplot2** \code{ggplot} object (default) or a \code{plotly/htmlwidget} if
#' \code{interactive=TRUE} is specified.
#'
#' @export
#'
...
gsnSubnetsDotPlot <- function(
  ...
  if( !is.null( label_width_chars ) ){
    if( interactive ){
      .y_scale <- ggplot2::scale_y_discrete( name = 'Title',
                                             labels = function( x ){
                                               gsub( pattern = "\\n",
                                                     replacement = "<br>",
    ...
    .y_scale <-  ggplot2::scale_y_discrete( name = 'Title', drop = FALSE )
  }
