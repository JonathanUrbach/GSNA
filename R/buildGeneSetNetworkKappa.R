...
#'
#' @description Using a gene set collection and a background of observable genes, calculate a
#' matrix of Cohen's kappa (*&kappa;*) coefficients and return a GSNData object.
#'
...
#'
#' @details This function wraps the process of creating a GSNData object and calculating a Cohen's
#' kappa coefficient matrix. The Cohen's *&kappa;* coefficient matrix is calculated using
#' \code{scoreKappaMatrix()}, which is implemented in C++.
#'
#' **Note:** Because with the *&kappa;* coefficient, higher values indicate a closer match between
...
buildGeneSetNetworkKappa <- function( object = NULL, ref.background = NULL, geneSetCollection = NULL, distMatrixFun = scoreKappaMatrix_C ){
  buildGeneSetNetworkGeneric(object, ref.background, geneSetCollection, distMatrixFun, distance = 'kappa', optimal_extreme = "max" )
}
