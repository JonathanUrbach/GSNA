...
#' \item{\code{$distances[[distance]]$layout}}{The layout of vertices, subsetted.}
#'
...
gsnSubset<- function( object, distance = NULL, vertex_names = c(), subnet_names = NULL ){
  ...
  for( .distance in names( object$distances ) ){
    GSN.subset$distances[[.distance]] <- list()

    #GSN.subset$distances[[.distance]]$vertices <- vertex_names
    # This way of subsetting the vertices should preserve the order of vertices in the original object.
    GSN.subset$distances[[.distance]]$vertices <-
      with( object$distances[[.distance]], vertices[vertices %in% vertex_names] )
    ...
    # Subset layout if it exists.
    if( ! is.null( object$distances[[.distance]]$layout ) && "matrix" %in% class( object$distances[[.distance]]$layout ) ){
      if ( ! is.null( object$distances[[.distance]]$igraph ) ){
        # Use ordering of vertices in igraph if available.
        GSN.subset$distances[[.distance]]$layout <-
          object$distances[[.distance]]$layout[ names( igraph::V( object$distances[[.distance]]$igraph ) ) %in% vertex_names,]
      } else {
        # Otherwise, assume that the ordering of vertices in layout is the same as in object$distances[[.distance]]$vertices.
        GSN.subset$distances[[.distance]]$layout <-
          object$distances[[.distance]]$layout[ object$distances[[.distance]]$vertices %in% vertex_names,]
      }
    }
  }
